...
ig.GAMEPAD = {
	...
	HOME: ig.GAMEPAD_BUTTON_OFFSET + 16,
	LA_LEFT: ig.GAMEPAD_BUTTON_OFFSET + 17,
	LA_RIGHT: ig.GAMEPAD_BUTTON_OFFSET + 18,
	LA_UP: ig.GAMEPAD_BUTTON_OFFSET + 19,
	LA_DOWN: ig.GAMEPAD_BUTTON_OFFSET + 20,
	RA_LEFT: ig.GAMEPAD_BUTTON_OFFSET + 21,
	RA_RIGHT: ig.GAMEPAD_BUTTON_OFFSET + 22,
	RA_UP: ig.GAMEPAD_BUTTON_OFFSET + 23,
	RA_DOWN: ig.GAMEPAD_BUTTON_OFFSET + 24
};
...
ig.Input.inject({
    ...
    pollGamepad: function() {
		...
		bind = this.bindings[ig.GAMEPAD_BUTTON_OFFSET + 21];
		if (this.gamepad.axes[5] < -this.treshold) {
			this.actions[bind] = true;
			this.presses[bind] = true;
		} else {
			this.delayedKeyup[bind] = true;
		}
		bind = this.bindings[ig.GAMEPAD_BUTTON_OFFSET + 22];
		if (this.gamepad.axes[5] > this.treshold) {
			this.actions[bind] = true;
			this.presses[bind] = true;
		} else {
		...
		bind = this.bindings[ig.GAMEPAD_BUTTON_OFFSET + 23];
		if (this.gamepad.axes[6] < -this.treshold) {
			this.actions[bind] = true;
			this.presses[bind] = true;
		} else {
			this.delayedKeyup[bind] = true;
		}
		bind = this.bindings[ig.GAMEPAD_BUTTON_OFFSET + 24];
		if (this.gamepad.axes[6] > this.treshold) {
			this.actions[bind] = true;
			this.presses[bind] = true;
		} else {
