...
wm.Weltmeister = ig.Class.extend({	
	...
	init: function() {
		...
		// --- AJOUT : Création de l'affichage des coordonnées dans le header ---
		// On ajoute un <span> aligné à droite dans la barre de menu
		$('#headerMenu').append('<span id="wm-coords">0, 0</span>');
		// ----------------------------------------------------------------------
	...
	mousemove: function() {
		...
		// --- AJOUT : Mise à jour des coordonnées ---
		if( typeof ig.input.mouse.x !== 'undefined' ) {
			var mx = ig.input.mouse.x + this.screen.x;
			var my = ig.input.mouse.y + this.screen.y;
			$('#wm-coords').text( 'X:' + Math.round(mx) + ' Y:' + Math.round(my) );
		}
		// ------------------------------------------
